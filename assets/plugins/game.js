    ...
    const res = MapGen.generate({
      ...
      callbacks: {
        ...
        placeBoss: (kind,tx,ty)=>{
          const px = tx * TILE + 8;
          const py = ty * TILE + 8;
          const bossEnt = spawnBossForLevel(level, px, py);
          if (!bossEnt) return;
        },
...
function drawEntities(c2){
  ...
  async function buildLevelForCurrentMode(levelNumber){
    ...
    const spawnCfg = payload?.levelRules?.level?.spawn;
    if (spawnCfg && Number.isFinite(spawnCfg.x) && Number.isFinite(spawnCfg.y)) {
      G.startControlRoom = {
        x: spawnCfg.x * tileSizeForStart,
        y: spawnCfg.y * tileSizeForStart,
      };
    } else if (G.mapAreas?.control) {
