(function(){
  ...
  function updateHeroDomBreath(st, dt, cfg = {}){
    ...
    const speed = animCfg.breathSpeed ?? cfg.breathSpeed ?? 0.45;
    const amp = animCfg.breathAmp ?? cfg.breathAmp ?? 0.05;
  ...
  function positionHeroOverlay(st, cam, e, worldX, worldY, scale, cfg = {}){
    ...
    const baseZ = Number.isFinite(cfg.zIndexBase) ? cfg.zIndexBase : 24;
    const zIndex = Math.min(96, baseZ + depth * 0.05);
