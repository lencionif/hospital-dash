...
(function (W) {
  ...
  const API = {
    ...
    spawn(x,y,props={}){
      ...
      ensureOnArrays(e);
      try {
      const puppet = window.Puppet?.bind?.(e, 'npc_female_base', { z: 0, scale: 1, data: { skin: e.skin || 'nurse' } })
        || window.PuppetAPI?.attach?.(e, { rig: 'npc_female_base', z: 0, scale: 1, data: { skin: e.skin || 'nurse' } });
